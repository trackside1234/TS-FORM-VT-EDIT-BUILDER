...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/8.0"}});
...
function lrPageMatches(html,date,venue){
  const t=lrNorm(html), v=lrNorm(venue);
  const [y,m,d]=date.split("-").map(Number);
  const months=["","january","february","march","april","may","june","july","august","september","october","november","december"];
  const full=`${d} ${months[m]} ${y}`;
  const short=`${d} ${months[m].slice(0,3)} ${y}`;
  return (t.includes(full)||t.includes(short)) && (!v || t.includes(v));
}
...
async function loveRacingResolve(date,venue,distance,horseName){
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/8.0"}});
      if(!r.ok) continue;
      const html=await r.text();
      if(!lrPageMatches(html,date,venue)) continue;
      const rn=lrRaceNumberFromHtml(html,distance,horseName);
      return {ok:true,meeting_id:id,venue,race_number:rn,url};
    }catch{}
  }
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:8});
