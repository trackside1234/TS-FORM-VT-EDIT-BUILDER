...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/16.0"}});
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{
        headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/16.0"},
        redirect:"follow"
      });
      if(!r.ok) continue;
  ...
  for(let raceNo=1; raceNo<=20; raceNo++){
    ...
    try{
      const r=await fetch(url,{
        headers:{
          accept:"text/html",
          "user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/16.0)"
        },
        redirect:"follow"
      });
      if(!r.ok) continue;
...
async function breednetResolve(date,trackCode,horseName,distance){
  const code=String(trackCode||"").trim().toLowerCase().replace(/[^a-z0-9]/g,"");
  const d=ddmmyyWorker(date);
  const horse=lrNorm(horseName);
  const dist=String(distance||"").replace(/\D/g,"");
  if(!code || !d || !horse) return {ok:false,reason:"missing code/date/horse"};

  for(let raceNo=1; raceNo<=20; raceNo++){
    const urls=[
      `https://www.breednet.com.au/race-meeting/${code}/${d}/${raceNo}`,
      `https://www.breednet.com.au/race-meeting/?raceno=${raceNo}&textdate=${d}&trackcode=${code.toUpperCase()}`
    ];
    for(const url of urls){
      try{
        const r=await fetch(url,{
          headers:{accept:"text/html","user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/16.0)"},
          redirect:"follow"
        });
        if(!r.ok) continue;
        const html=await r.text();
        if(!lrNorm(html).includes(horse)) continue;

        let distanceMatch=null;
        if(dist) distanceMatch=new RegExp(`\\b${dist}\\s*m\\b`,"i").test(lrStrip(html));
        return {
          ok:true,race_number:raceNo,track_code:code.toUpperCase(),url,
          source:"breednet_race_meeting_horse_match",distanceMatch
        };
      }catch{}
    }
  }
  return {ok:false,track_code:code.toUpperCase(),source:"breednet_no_match"};
}
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:16});
      ...
      if(url.pathname==="/api/resolve-form-race"){
        ...
        const diagnostics={
          date, venue, track_code:trackCode, distance, horse_id:horseId, horse_name:horseName, start_id:startId,
        ...
        if(country==="NZ" && trackCode){
          try{
            const br=await breednetResolve(date,trackCode,horseName,distance);
