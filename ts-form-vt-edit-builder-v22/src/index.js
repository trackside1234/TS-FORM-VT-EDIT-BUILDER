...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/22.0"}});
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{
        headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/22.0"},
        redirect:"follow"
      });
      if(!r.ok) continue;
  ...
  for(let raceNo=1; raceNo<=20; raceNo++){
    ...
    try{
      const r=await fetch(url,{
        headers:{
          accept:"text/html",
          "user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/22.0)"
        },
        redirect:"follow"
      });
      if(!r.ok) continue;
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:22});
