...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/14.0"}});
...
async function loveRacingResolve(date,venue,distance,horseName){
  ...
  // Identify matching meeting pages by date; readable venue is a bonus, not a gate.
  for(const off of offsets){
    const id=estimate+off;
    if(id<1) continue;
    const url=`https://loveracing.nz/RaceInfo/${id}/Meeting-Overview.aspx`;
    try{
      const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/14.0"}});
      if(!r.ok) continue;
      const html=await r.text();
      if(!lrPageMatchesDate(html,date)) continue;
      const humanVenue=lrVenueFromHtml(html) || venue;
      const actual=lrNorm(humanVenue), incoming=lrNorm(venue);
      let score=1;
      if(actual && incoming && (actual.includes(incoming)||incoming.includes(actual))) score+=4;
      meetings.push({meeting_id:id,venue:humanVenue,url,score});
    }catch{}
  }
  meetings.sort((a,b)=>b.score-a.score);

  // Probe multiple historical race-detail URL styles. LOVERACING has used more than one route shape.
  const detailTemplates=[
    (mid,rn)=>`https://loveracing.nz/RaceInfo/${mid}/${rn}/Race-Detail.aspx`,
    (mid,rn)=>`https://loveracing.nz/RaceInfo/${mid}/${rn}/Race-Detail`,
    (mid,rn)=>`https://loveracing.nz/RaceInfo/${mid}/Race-${rn}.aspx`
  ];

  for(const mtg of meetings.slice(0,8)){
    for(let raceNo=1; raceNo<=20; raceNo++){
      for(const buildUrl of detailTemplates){
        const url=buildUrl(mtg.meeting_id,raceNo);
        try{
          const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/14.0"},redirect:"follow"});
          if(!r.ok) continue;
          const html=await r.text();
          const text=lrNorm(html);

          // Require exact normalized horse tokens to appear.
          if(!horse || !text.includes(horse)) continue;

          // Stronger guard: page should look like a race/result page.
          const plain=lrStrip(html);
          const looksRace=/result|race|runner|placing|finish|jockey/i.test(plain);
          if(!looksRace) continue;

          let distanceMatch=true;
          if(dist) distanceMatch=new RegExp(`\\b${dist}\\s*m\\b`,"i").test(plain);

          const detailVenue=lrVenueFromHtml(html) || mtg.venue || venue;
          return {
            ok:true,
            ...
            race_number:raceNo,
            url,
            source:"race_detail_horse_match",
            distanceMatch
          };
        }catch{}
      }
  ...
  // Do not invent a race number. Return meeting/venue only if that's all we can prove.
  if(meetings.length){
    return {
      ok:true,
      meeting_id:meetings[0].meeting_id,
      venue:meetings[0].venue||venue,
      race_number:null,
      url:meetings[0].url,
      source:"meeting_only"
    };
  ...
  for(let raceNo=1; raceNo<=20; raceNo++){
    ...
    try{
      const r=await fetch(url,{
        headers:{
          accept:"text/html",
          "user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/14.0)"
        },
        redirect:"follow"
      });
      if(!r.ok) continue;
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:14});
      ...
      if(url.pathname==="/api/resolve-form-race"){
        ...
        // Final race-number fallback: Racing & Sports uses stable historical URLs by
        // human venue/date/race number. Probe R1-R20 and identify the page containing the horse.
        try{
          const humanVenue =
            diagnostics?.loveracing?.venue ||
            venue;
          const ras=await racingAndSportsResolve(date,humanVenue,horseName,distance);
          diagnostics.racing_and_sports=ras;
          if(ras.ok && ras.race_number){
            return jsonResponse({
              method:"racing_and_sports_verified",
              fallback:true,
              inferred:true,
              diagnostics,
              event:{
                race:{
                  event_id:"",
                  meeting_id:"",
                  meeting_name:ras.venue||humanVenue,
                  display_meeting_name:ras.venue||humanVenue,
                  venue_name:ras.venue||humanVenue,
                  track:ras.venue||humanVenue,
                  description:"",
                  race_number:ras.race_number,
                  race_date_nz:date,
                  distance:Number(distance)||"",
                  track_condition:"",
                  class:"",
          ...
          diagnostics.racing_and_sports_error=err?.message||String(err);
        }
