...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/11.0"}});
...
async function loveRacingResolve(date,venue,distance,horseName){
  ...
  // Step 1: identify plausible meeting(s) by date.
  for(const off of offsets){
    const id=estimate+off;
    if(id<1) continue;
    const url=`https://loveracing.nz/RaceInfo/${id}/Meeting-Overview.aspx`;
    try{
      const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/11.0"}});
      if(!r.ok) continue;
      ...
      if(humanVenue) score+=1;
      meetings.push({meeting_id:id,venue:humanVenue,url,score});
    }catch{}
  }

  // Prefer meetings whose readable venue resembles the incoming venue, but do not require it.
  meetings.sort((a,b)=>b.score-a.score);

  // Step 2: inspect final race-detail/result pages, not the provisional overview.
  // This is the authoritative race-number recovery path.
  for(const mtg of meetings.slice(0,6)){
    for(let raceNo=1; raceNo<=20; raceNo++){
      const url=`https://loveracing.nz/RaceInfo/${mtg.meeting_id}/${raceNo}/Race-Detail.aspx`;
      try{
        const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/11.0"}});
        if(!r.ok) continue;
        const html=await r.text();
        ...
        // Ignore generic/error pages and require the selected horse to actually appear.
        if(!horse || !text.includes(horse)) continue;

        // Verify distance where available, but horse match is the strongest signal.
        ...
        // Also recover the human-readable venue directly from the race detail page if possible.
  ...
  // Last resort: retain best meeting info, but do not invent a race number.
  if(meetings.length){
    return {
      ok:true,
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:11});
