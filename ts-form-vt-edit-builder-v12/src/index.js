...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/12.0"}});
...
async function loveRacingResolve(date,venue,distance,horseName){
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/12.0"}});
      if(!r.ok) continue;
  ...
  for(const mtg of meetings.slice(0,8)){
    for(let raceNo=1; raceNo<=20; raceNo++){
      for(const buildUrl of detailTemplates){
        ...
        try{
          const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/12.0"},redirect:"follow"});
          if(!r.ok) continue;
          const html=await r.text();
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:12});
