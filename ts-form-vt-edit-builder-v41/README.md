...
## v37
Fixes HRNZ history `url2` temporal-dead-zone error by using a dedicated public-racebook URL variable in the TAB fallback route.


## v38 — HRNZ historical results resolver

Harness historical form no longer depends on TAB UUIDs or a hard-coded public-racebook prefix list. Completed Harness races now resolve by date + venue + horse name against HRNZ public results. The resolver first asks the authenticated HRNZ API for the meeting/club suffix where possible, uses verified suffixes, and only then probes the remaining 00–99 result suffixes in controlled batches. It verifies the returned page belongs to the requested venue before extracting actual Race 1/Race 2/etc. numbers containing the horse. Upcoming/current Harness meetings continue to use the authenticated HRNZ horse-starts API.


## v39 — Harness HRNZ 403 fallback fix

Harness history now preserves usable TAB `last_starts` while HRNZ history is being checked.

Changes:
- Existing TAB historical starts remain visible immediately when a Harness runner is expanded.
- HRNZ history is still attempted in the background.
- Successful HRNZ public-results or authenticated history replaces the TAB history.
- HRNZ 403/errors no longer blank a runner's existing history.
- When HRNZ fails but TAB history exists, the UI shows a small warning and retains the TAB history instead of replacing it with an error.
- If there is no existing history and HRNZ also fails, the actual HRNZ error remains visible.
- Harness historical rows identify their actual source rather than always labelling every row `HRNZ racebook`.
- Thoroughbred behaviour is unchanged.


## v40 — fixed the actual cause of history collapsing to a bare HRNZ 403

Two real bugs found and fixed.

### 1. `hrnzFetchPublicResultPage` referenced an undefined `env`
It called `hrnzJSON(env, ...)` without `env` ever being passed into the function.
This threw a `ReferenceError` on every call, silently swallowed by an empty
`catch{}`, so the "ask HRNZ for the meeting's clubId" optimisation never ran and
every public-results lookup fell back to the slow 00–99 brute-force probe.
`env` is now a real parameter, passed through from `hrnzPublicHistoricalHorse`.

### 2. `ensureHarnessHistory` looked up the wrong date/venue (the actual v39-vs-now bug)
When a Harness runner was expanded, the app tried to enrich its history by
calling HRNZ public results using the **current/upcoming race's own date and
venue** - not the date/venue of each individual past start. Since the upcoming
race is normally in the future, that lookup could essentially never match
anything, so it fell straight through to the authenticated
`/api/hrnz-horse-starts` endpoint (known to 403 for some accounts). That is
what produced the "shows TAB history with R? for a second, then collapses to
HRNZ racebook history error: HRNZ 403" symptom.

The resolver now:
- enriches each existing start (TAB or HRNZ) using **that start's own** date
  and venue, never the current race's;
- resolves race number/name per-start, independently, so one failed lookup
  doesn't affect the others;
- only calls the known-403 authenticated endpoint as a last resort, and only
  when there is no existing history at all to preserve;
- never replaces a non-empty history list with an error - a soft warning is
  shown instead if some individual starts couldn't be enriched.

Thoroughbred behaviour is unchanged.

## v41 — Synthetic venue labels + unverified-field highlighting

Two feature requests from the team using the live tool.

### 1. Synthetic track venue labels
"Riccarton Synthetic", "Awapuni Synthetic" and "Cambridge Synthetic" now
resolve to just "Riccarton" / "Awapuni" / "Cambridge" in the venue key text.
Internal codes are unaffected (they already mapped correctly). This only
fires on automatic resolution - if you manually type "Riccarton Synthetic"
into the venue field yourself, it's left alone.

### 2. Bold red highlighting for unverified code / race number / venue
In the final edit sheet (on-screen preview, rich clipboard copy, and Word
doc export), only the specific pieces of information that couldn't be
verified are now shown in bold red, so they're easy to spot when scanning a
big sheet later:
- the internal media **code**, if it's still the "RACE" placeholder;
- the **race number**, if it's still "R??";
- the **venue** text, if it can't be matched to any known internal code.

Everything else on the same line (condition, distance, date) stays normal
text. Sheet entries now also remember which category (T/H/G) they were
created under, so this check stays correct even if you switch tabs after
adding a race to the sheet.

Also fixed two edge cases in the v40 venue fuzzy-matcher found via a wider
regression sweep: an ambiguous-match scoring bug where an unqualified
"Canterbury" query preferred "South Canterbury" over the exact "Canterbury"
match, and a leftover crude substring tier that let "Sth Canterbury" wrongly
match plain "Canterbury" by ignoring the "Sth" qualifier entirely. Both
fixed; the regression suite (now 15 cases) passes clean.
