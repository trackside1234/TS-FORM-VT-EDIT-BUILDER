...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/4.0"}});
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:4});
      ...
      if(url.pathname==="/api/resolve-form-race"){
        ...
        // Some older dates are rejected by /racing/list. Try the meetings endpoint as a second
