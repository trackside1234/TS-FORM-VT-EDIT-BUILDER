...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/9.0"}});
...
async function loveRacingResolve(date,venue,distance,horseName){
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/9.0"}});
      if(!r.ok) continue;
      const html=await r.text();
      if(!lrPageMatchesDate(html,date)) continue;

      const humanVenue=lrVenueFromHtml(html);
      const pageText=lrNorm(html);
      const horseHit=horse && pageText.includes(horse);
      const rn=lrRaceNumberFromHtml(html,distance,horseName);

      let score=0;
      if(horseHit) score+=10;
      if(rn) score+=5;
      if(dist && new RegExp(`\b${dist}\s*m\b`,"i").test(lrStrip(html))) score+=2;
      if(humanVenue) score+=1;

      found.push({ok:true,meeting_id:id,venue:humanVenue||venue,race_number:rn,url,score,horseHit});
    }catch{}
  }
  found.sort((a,b)=>b.score-a.score);
  if(found.length && found[0].score>=6) return found[0];
  return {ok:false,estimate,candidates:found.slice(0,3)};
}
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:9});
