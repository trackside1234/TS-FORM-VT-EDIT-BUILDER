...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/28.0"}});
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{
        headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/28.0"},
        redirect:"follow"
      });
      if(!r.ok) continue;
  ...
  for(let raceNo=1; raceNo<=20; raceNo++){
    ...
    try{
      const r=await fetch(url,{
        headers:{
          accept:"text/html",
          "user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/28.0)"
        },
        redirect:"follow"
      });
      if(!r.ok) continue;
...
function hrnzMeetingName(m){
  ...
  if(/^NZ\s+(Metro|Metropolitan)(\s+TC)?$/i.test(raw)) return "ADDINGTON";
  if(/^Addington(\s+Raceway)?$/i.test(raw)) return "ADDINGTON";
  if(/^Invcargill$/i.test(raw) || /^Invercargill$/i.test(raw) || /^Ascot\s+Park(\s+Raceway)?$/i.test(raw)) return "INVERCARGILL";
  return raw;
}
function hrnzCompactStart(s){
  const rh=s?.raceHeader||{};
  const result=s?.horseResult||{};
  const venueRaw=rh?.track?.name||rh.clubName||"";
  let venue=String(venueRaw||"").trim();
  if(/^NZ\s+(Metro|Metropolitan)(\s+TC)?$/i.test(venue)) venue="ADDINGTON";
  if(/^Invcargill$/i.test(venue) || /^Invercargill$/i.test(venue) || /^Ascot\s+Park(\s+Raceway)?$/i.test(venue)) venue="INVERCARGILL";
  return {
    id:`hrnz:${rh.meetingId||""}:${rh.raceId||""}`,
    meeting_id:rh.meetingId||"",
    race_id:rh.raceId||"",
    ...
    class:rh.raceDescriptionAbbreviated||rh.raceNameDetails||rh.raceName||"",
    race_name:rh.raceName||"",
    description:rh.raceName||"",
...
async function hrnzHorseStarts(env,horseId){
  ...
  return Array.isArray(starts)?starts.map(hrnzCompactStart):[];
}
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:28});
