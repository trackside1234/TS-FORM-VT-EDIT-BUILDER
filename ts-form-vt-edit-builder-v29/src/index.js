...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/29.0"}});
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{
        headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/29.0"},
        redirect:"follow"
      });
      if(!r.ok) continue;
  ...
  for(let raceNo=1; raceNo<=20; raceNo++){
    ...
    try{
      const r=await fetch(url,{
        headers:{
          accept:"text/html",
          "user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/29.0)"
        },
        redirect:"follow"
      });
      if(!r.ok) continue;
...
async function hrnzFindRaceMetaById(env,raceDate,raceId){
  ...
  try{
    const meetings=await hrnzJSON(env,"/racing/meetings",{
      startDate:raceDate,endDate:raceDate,meetingType:"OFFICIAL"
    });
    for(const mh of (Array.isArray(meetings)?meetings:[])){
      const meetingId=mh?.meetingId;
      if(!meetingId) continue;
      let detail;
      try{detail=await hrnzJSON(env,`/racing/meetings/${encodeURIComponent(meetingId)}`)}
      catch{continue}
      for(const rh of (Array.isArray(detail?.races)?detail.races:[])){
        if(String(rh?.raceId||"")===rid){
          const meta={
            meetingId,
            raceId:rh.raceId,
            raceNumber:rh.raceNumber||"",
            raceDate:detail?.meetingHeader?.meetingDate||raceDate,
            venue:hrnzCanonicalVenue(detail?.meetingHeader?.track?.name||detail?.meetingHeader?.clubName||""),
            raceName:rh.raceName||rh.raceDescriptionAbbreviated||"",
...
async function hrnzNormalizeStarts(env,starts){
  const arr=Array.isArray(starts)?starts:[];
  const out=[];
  for(const s of arr){
    const rh=s?.raceHeader||{};
    let meta=null;
    if(!rh.raceNumber || !rh.meetingId){
      meta=await hrnzFindRaceMetaById(env,rh.raceDate||"",rh.raceId||"");
    }
...
function hrnzCompactStart(s,meta=null){
  const rh=s?.raceHeader||{};
  const result=s?.horseResult||{};
  const venue=hrnzCanonicalVenue(rh?.track?.name||meta?.venue||rh.clubName||"");
  const raceNumber=rh.raceNumber||meta?.raceNumber||"";
  const meetingId=rh.meetingId||meta?.meetingId||"";
  const raceId=rh.raceId||meta?.raceId||"";
  return {
    id:`hrnz:${meetingId}:${raceId}`,
    meeting_id:meetingId,
    race_id:raceId,
    race_number:raceNumber,
    date:rh.raceDate||meta?.raceDate||"",
    distance:hrnzDistance(rh)||meta?.distance||"",
    track:venue,venue_name:venue,
    track_condition:rh?.track?.trackCondition||"",
    class:rh.raceDescriptionAbbreviated||rh.raceNameDetails||rh.raceName||meta?.raceName||"",
    race_name:rh.raceName||meta?.raceName||"",
    description:rh.raceName||meta?.raceName||"",
    finish:result?.placing||"",
    driver:result?.driver?.driverName||"",
    driver_name:result?.driver?.driverName||"",
    trainer:result?.trainer?.trainerName||"",
    trainer_name:result?.trainer?.trainerName||"",
    review:"",
    gait:result?.gaitInUse||rh.raceGait||"",
    start_type:rh.startType||"",
    source:"hrnz",
    race_number_source:rh.raceNumber?"horse_start":"race_id_lookup"
  };
}
async function hrnzHorseStarts(env,horseId){
  const starts=await hrnzJSON(env,`/equine/horses/${encodeURIComponent(horseId)}/raceStarts`,{
    raceType:"OFFICIAL",
    numberOfStarts:12
  });
  return hrnzNormalizeStarts(env,starts);
}
...
async function hrnzCompactEvent(env,meetingId,raceId){
  ...
  const runners=await Promise.all(rawRunners.map(async x=>{
    let starts=[];
    try{starts=await hrnzHorseStarts(env,x.horseId)}catch{}
    const result=x?.result||{};
    return {
      entrant_id:String(x.horseId||""),
      horse_id:String(x.horseId||""),
      runner_number:x.bookNumber||"",
      name:x.horseName||"",
      is_scratched:!!x.scratched,
      jockey:"",
      driver:x?.driver?.driverName||"",
      driver_name:x?.driver?.driverName||"",
      trainer:x?.trainer?.trainerName||"",
      trainer_name:x?.trainer?.trainerName||"",
      silk_url_64x64:x?.colours?.coloursURL||"",
      silk_url_128x128:x?.colours?.coloursURL||"",
      ...
      preview:"",
      result:{
        position:parseInt(result.placing)||null,
        runner_number:x.bookNumber||"",
        name:x.horseName||""
      }
    };
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:29});
