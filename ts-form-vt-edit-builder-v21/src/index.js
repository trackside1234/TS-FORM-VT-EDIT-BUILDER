...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/21.0"}});
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{
        headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/21.0"},
        redirect:"follow"
      });
      if(!r.ok) continue;
  ...
  for(let raceNo=1; raceNo<=20; raceNo++){
    ...
    try{
      const r=await fetch(url,{
        headers:{
          accept:"text/html",
          "user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/21.0)"
        },
        redirect:"follow"
      });
      if(!r.ok) continue;
...
function hrnzMeetingName(m){
  return m?.track?.name || m?.trackName || m?.clubName || "Harness Meeting";
}
function hrnzCompactStart(s){
  ...
  return {
    ...
    track:rh?.track?.name||rh.clubName||"",
    venue_name:rh?.track?.name||rh.clubName||"",
    ...
    finish:s?.placing||"",
    driver:s?.driver?.driverName||"",
    driver_name:s?.driver?.driverName||"",
    trainer:s?.trainer?.trainerName||"",
    review:"",
    gait:s?.gaitInUse||rh.raceGait||"",
...
async function hrnzCompactEvent(env,meetingId,raceId){
  ...
  return {
    race:{
      ...
      venue_name:mh?.track?.name||hrnzMeetingName(mh),
      track:mh?.track?.name||hrnzMeetingName(mh),
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:21});
