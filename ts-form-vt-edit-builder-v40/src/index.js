...
async function tabJSON(url){
  const r = await fetch(url,{headers:{accept:"application/json","user-agent":"TS-Form-VT-Edit-Builder/38.0"}});
  ...
  for(const off of offsets){
    ...
    try{
      const r=await fetch(url,{
        headers:{accept:"text/html","user-agent":"TS-Form-VT-Edit-Builder/38.0"},
        redirect:"follow"
      });
      if(!r.ok) continue;
  ...
  for(let raceNo=1; raceNo<=20; raceNo++){
    ...
    try{
      const r=await fetch(url,{
        headers:{
          accept:"text/html",
          "user-agent":"Mozilla/5.0 (compatible; TS-Form-VT-Edit-Builder/38.0)"
        },
        redirect:"follow"
      });
      if(!r.ok) continue;
...
const hrnzPublicMeetingCache=new Map();
const hrnzPublicPageCache=new Map();

function hrnzVenueMatches(a,b){
  const x=norm(hrnzCanonicalVenue(a)), y=norm(hrnzCanonicalVenue(b));
  if(!x||!y)return false;
  if(x===y)return true;
  const aliases={
    addington:['nzmetropolitan','nzmetro','newzealandmetropolitantrottingclub'],
    cambridge:['waikbop','waikatobayofplenty','waikatobayofplentyharness'],
    invercargill:['invcargill','ascotpark'],
    ashburton:['ashburtontrottingclub'],
    rangiora:['rangioraharness'],
    manawatu:['manawatuharness'],
    winton:['wintonharness'],
    oamaru:['oamaruharness'],
  };
  for(const [k,vals] of Object.entries(aliases)){
    if((x===k&&vals.includes(y))||(y===k&&vals.includes(x)))return true;
  }
  return x.includes(y)||y.includes(x);
}

function hrnzResultMeetingTitle(lines){
  const i=lines.findIndex(x=>/^#\s*/.test(x));
  return i>=0?lines[i].replace(/^#\s*/,'').trim():lines[0]||'';
}

function hrnzHorseInBlock(lines,horseName,start,end){
  const target=norm(horseName);
  if(!target)return false;
  for(let i=start;i<end;i++){
    const n=norm(lines[i]);
    if(n===target)return true;
    if(n.includes(target)&&target.length>=5)return true;
  }
  return false;
}

function hrnzParsePublicResultsForHorse(html,date,venue,horseName){
  const lines=hrnzHtmlToLines(html);
  const title=hrnzResultMeetingTitle(lines);
  if(!hrnzVenueMatches(title,venue) && !lines.slice(0,80).some(x=>hrnzVenueMatches(x,venue))) return null;
  const starts=[];
  const raceMarkers=[];
  for(let i=0;i<lines.length;i++){
    const m=lines[i].match(/^Race Number:\s*$/i);
    if(m){
      const next=lines[i+1]||'';
      const rm=next.match(/^Race\s+(\d+)$/i);
      if(rm)raceMarkers.push({i,n:Number(rm[1])});
    }
  }
  for(let k=0;k<raceMarkers.length;k++){
    const a=raceMarkers[k].i, b=k+1<raceMarkers.length?raceMarkers[k+1].i:lines.length;
    if(!hrnzHorseInBlock(lines,horseName,a,b))continue;
    const span=lines.slice(a,b);
    let raceName='', distance='', condition='', classText='';
    const rpos=span.findIndex(x=>/^Race\s+\d+$/i.test(x));
    if(rpos>=0){
      raceName=span[rpos+1]||'';
      const desc=span[rpos+2]||'';
      const dm=desc.match(/(\d{3,4})m\b/i);
      if(dm){distance=dm[1];classText=desc.replace(/\s+/g,' ').trim()}
      for(const x of span.slice(rpos+1,rpos+8)){
        if(!condition && /^(Fast|Good|Dead|Easy|Slushy|Wet|Heavy|Muddy|Slow)$/i.test(x))condition=x;
      }
    }
    let finish='';
    for(let i=rpos+1;i<Math.min(span.length,rpos+25);i++){
      const m=span[i].match(/^(\d+)\s+\|/);
      if(m){
        // The participant table begins with the placing. Find the row containing the horse.
        if(norm(span[i]).includes(norm(horseName))){finish=m[1];break}
        if(i+1<span.length && norm(span[i+1]).includes(norm(horseName))){finish=m[1];break}
      }
    }
    starts.push({
      id:`public:${date}:${venue}:${raceMarkers[k].n}`,
      meeting_id:'',race_id:'',race_number:String(raceMarkers[k].n),date,
      distance,track:hrnzCanonicalVenue(venue),venue_name:hrnzCanonicalVenue(venue),
      track_condition:condition,class:classText,race_name:raceName,description:raceName,
      finish,driver:'',driver_name:'',trainer:'',trainer_name:'',review:'',
      source:'hrnz_public_results',race_number_source:'public_results_resolver'
    });
  }
  return starts.length?{title,starts}:null;
}

async function hrnzFetchPublicResultPage(env,date,venue,horseName){
  const dp=hrnzDateParts(date);
  if(!dp)throw new Error('Invalid historical race date');
  const cacheKey=`${dp.iso}|${norm(venue)}`;
  if(hrnzPublicPageCache.has(cacheKey)){
    const cached=hrnzPublicPageCache.get(cacheKey);
    const parsed=hrnzParsePublicResultsForHorse(cached.html,dp.iso,venue,horseName);
    if(parsed)return {...parsed,url:cached.url,code:cached.code,cached:true};
  }

  // First ask the authenticated HRNZ API for the meeting and, where available,
  // use its clubId as the two-digit public-results suffix. This is a nice-to-have
  // optimization only - if HRNZ auth is not configured or fails, we silently
  // fall through to the known-suffix table and brute-force probe below.
  let candidates=[];
  try{
    const lp=await hrnzJSON(env,'/racing/meetings',{startDate:dp.iso,endDate:dp.iso,meetingType:'OFFICIAL'});
    const arr=Array.isArray(lp)?lp:(lp?.meetings||lp?.data||lp?.data?.meetings||[]);
    for(const mh of arr){
      const name=mh?.clubName||mh?.track?.name||mh?.trackName||'';
      if(hrnzVenueMatches(name,venue)){
        const c=mh?.clubId ?? mh?.club?.clubId ?? mh?.club?.id ?? mh?.track?.clubId;
        if(c!==undefined && /^\d+$/.test(String(c))) candidates.push(String(c).padStart(2,'0').slice(-2));
        const mid=mh?.meetingId||mh?.id;
        if(mid){
          try{
            const detail=await hrnzJSON(env,`/racing/meetings/${encodeURIComponent(mid)}`);
            const hh=detail?.meetingHeader||detail?.meeting||mh;
            const cc=hh?.clubId ?? hh?.club?.clubId ?? hh?.club?.id ?? hh?.track?.clubId;
            if(cc!==undefined && /^\d+$/.test(String(cc))) candidates.unshift(String(cc).padStart(2,'0').slice(-2));
          }catch{}
        }
      }
    }
  ...
  // Known verified HRNZ public-result suffixes from the working examples.
  const known={invercargill:'57',addington:'32',nzmetropolitan:'32',nzmetro:'32',cambridge:'09',waikbop:'09',waikatobayofplenty:'09',winton:'55'};
  const nk=norm(hrnzCanonicalVenue(venue));
  if(known[nk]) candidates.push(known[nk]);
  candidates=[...new Set(candidates)];

  // Only if HRNZ did not expose the suffix do we probe the remaining 00–99 space.
  // Limit concurrency so a single historical lookup does not hammer HRNZ.
  const tryCode=async code=>{
    const url=`https://infohorse.hrnz.co.nz/datahrs/results/${dp.mm}${dp.dd}${code}rs.htm`;
    try{
      const r=await fetch(url,{headers:{accept:'text/html,application/xhtml+xml','user-agent':'Mozilla/5.0'}});
      if(!r.ok)return null;
      const html=await r.text();
      const parsed=hrnzParsePublicResultsForHorse(html,dp.iso,venue,horseName);
      if(parsed)return {code,url,html,...parsed};
    }catch{}
    return null;
  };

  for(const code of candidates){
    const hit=await tryCode(code);
    if(hit){hrnzPublicPageCache.set(cacheKey,{html:hit.html,url:hit.url,code:hit.code});return hit;}
  }

  const all=[...Array(100)].map((_,i)=>String(i).padStart(2,'0')).filter(x=>!candidates.includes(x));
  const batchSize=8;
  for(let i=0;i<all.length;i+=batchSize){
    const batch=await Promise.all(all.slice(i,i+batchSize).map(tryCode));
    const hit=batch.find(Boolean);
    if(hit){hrnzPublicPageCache.set(cacheKey,{html:hit.html,url:hit.url,code:hit.code});return hit;}
  }
  throw new Error(`Could not locate HRNZ public results for ${venue} on ${dp.iso}`);
}

async function hrnzPublicHistoricalHorse(env,date,venue,horseName){
  const hit=await hrnzFetchPublicResultPage(env,date,venue,horseName);
  return {ok:true,source:'hrnz_public_results',url:hit.url,club_code:hit.code,count:hit.starts.length,starts:hit.starts};
}
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:40});
      ...
      if(url.pathname==="/api/hrnz-public-historical-horse"){
        ...
        if(!date||!venue||!horseName)return jsonResponse({error:"date, venue and horse_name required"},400);
        try{return jsonResponse(await hrnzPublicHistoricalHorse(env,date,venue,horseName));}
