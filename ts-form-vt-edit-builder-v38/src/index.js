...
async function hrnzFetchPublicResultPage(date,venue,horseName){
  ...
  // use its clubId as the two-digit public-results suffix.
  let candidates=[];
  try{
...
async function hrnzPublicHistoricalHorse(env,date,venue,horseName){
  const hit=await hrnzFetchPublicResultPage(date,venue,horseName);
...
export default {
  async fetch(request, env){
    ...
    try{
      if(url.pathname==="/api/health") return jsonResponse({ok:true,service:"ts-form-vt-edit-builder",version:38});
